  ...
  set_up_html: ->
    ...
    if @is_multiple
      ...
      @container.html '<div class="' +
        ...
        '"><div class="search-field"><input role="combobox" aria-expanded="false" aria-owns="' + container_props.id + '_results" type="text" value="' +
        this.escape_html(@default_text) + '" class="default" autocomplete="off" style="width:25px;" /></div></div><div class="' +
        this.escape_html(drop_classes.join(' ')) + '"><ul role="listbox" id="' + container_props.id + '_results" class="chosen-results"></ul></div>'
    else
